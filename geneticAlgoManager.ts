...
import { PipePair } from '../game/gameObjects/pipePair';
import { NeuralNetwork } from './network/neuralnetwork';

export class GeneticAlgoManager {
  generation = 1;
  prevAvgScore = 0;
  avgScore = 0;
  birdsAlive = 0;
  ...
  computeNextGeneration(birds: Bird[]) {
    ...
    let fittestBirds: Bird[] = this.getFittestBirds(birds, birdsToKeep);
    ...
    for (let i = 0; i < this.allTimeBest.length; i++) {
      ...
      for (let j = 0; j < numOfMutatedBirdsPerATB; j++) {
        let newBird = new Bird(this.allTimeBest[i].id + '*');
        newBird.brain = newAi.deepCopy();
    ...
    for(let i = 0; i < numOfCrossovers; i++){
      ...
      let newBird = new Bird("crossover - "+i);
      newBird.brain = newAi;
    ...
    for (let i = birds.length; i < this.numOfAgents; i++) {
      ...
      let newBird = new Bird(i + '');
      newBird.brain = newAi;
      newBird.brain.mutate(
        (0.5 * (i / this.numOfAgents)) / (.2*this.generation),
      );
      birds.push(newBird);
    }
  ...
  getFittestBirds(birds: Bird[], topX: number) {
    let newBirds: Bird[] = [];
  ...
  updateBestBirds(newFittestBirds: Bird[]) {
  ...
  getProbabilityIndex(fittestBirds: Bird[], probabilities: number[]) {
  ...
  crossover(birdA: Bird, birdB: Bird){
